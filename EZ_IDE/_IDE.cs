...
namespace EZ_IDE
{
    public partial class _IDE
    {
        ...
        public _IDE()
        {
            ...
            Settings.Initialize(ezcode, null, null);
        ...
        private async void Start_Click(object sender, EventArgs e) // START
        {
            try
            {
                ezcode.SetScriptDirectory(Settings.Directory);
                if (!Settings.EZProj) await ezcode.Play(InputText.Text);
                else await ezcode.PlayFromProj(new EZProj(InputText.Text, Settings.Directory));
            }
            catch (Exception ex)
            {
